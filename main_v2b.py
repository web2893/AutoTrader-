...
class BacktestEngine:
    ...
    def run_backtest(self, data: pd.DataFrame, strategy) -> Dict:
        self.reset()
        strategy.init(data)  # 初始化策略时生成Renko数据
...
class StrategyAdapter:
    ...
    def init(self, data: pd.DataFrame):
        # 初始化时生成完整的Renko数据
        self.strategy.generate_renko(data)
        
    def generate_signals(self, row: pd.Series, engine: BacktestEngine) -> List[Dict]:
        current_time = row.name
        renko_data = self.strategy.renko_data
        
        # 找到最后一个时间戳小于等于当前时间的砖块
        mask = renko_data['time'] <= current_time
        if not mask.any():
            return []
        current_brick = renko_data[mask].iloc[-1]
        
        # 判断方向变化
        if current_brick['direction'] != self.strategy.current_direction:
            self.strategy.current_direction = current_brick['direction']
            if current_brick['direction'] == 'up':
                return [{
                    "timestamp": current_time,
                    "type": "buy",
                    "quantity": self.strategy.fixed_quantity
                }]
            elif current_brick['direction'] == 'down':
                return [{
                    "timestamp": current_time,
                    "type": "sell",
                    "quantity": self.strategy.fixed_quantity
